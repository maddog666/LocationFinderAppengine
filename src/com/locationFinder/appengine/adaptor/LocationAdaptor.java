...
public class LocationAdaptor {
	...
	public List<Location> fetchLocations(float lat, float lng, float radius) {
		...
		for(Entity locationEntity: pq.asIterable()){	
			//fetch all categories with the category key
			Location location = this.createLocation(locationEntity);
			locations.add(location);			
		}		
	...
	public List<Location> fetchLocations(float lat, float lng, String category, float radius) {
		List<Location> locations = new ArrayList<Location>();
		return this.fetchLocations(lat, lng, radius);
		//return locations;
	}
	
	public List<Location> fetchLocations(String categoryString, float radius) {
		List<Location> locations = new ArrayList<Location>();
		Query query = new Query("Categories");
		query.addFilter("category", 
				Query.FilterOperator.EQUAL, categoryString);
		PreparedQuery pq = this.getDatastore().prepare(query);
		Entity categoryEntity = pq.asSingleEntity();
		Key categoryEntityKey = categoryEntity.getKey();
		
		query = new Query("LocationCategory");
		query.addFilter("categoryKey", 
				Query.FilterOperator.EQUAL, categoryEntityKey);
		pq = this.getDatastore().prepare(query);
		for(Entity result: pq.asIterable()){	
			//fetch all categories with the category key
			Key locationKey = (Key) result.getProperty("locationKey");				
			query = new Query("Location", locationKey);				
			PreparedQuery pqLocation = this.getDatastore().prepare(query);
			Entity locationEntity = pqLocation.asSingleEntity();
